...
class Condition(models.Model):
    ...
    def __unicode__(self):
        ...
        while p is not None:
            ...
            ancestors.insert(0, p.condition_opt)
            p = p.parent_condition
        return "{}: {} -> {}".format(transition, ancestors, self.condition_opt)

    def check_condition(self, *, object_id, user, object_state, transition=None):
        if self.condition_opt == "function":
